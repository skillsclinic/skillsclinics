...
+function ($) {
	...
	var Time = function Time(hour, minute) {
		this.hour = hour;
		this.minute = minute;

		this.format = function (format, hourPadding) {
			var that = this,
			    is24Hour = (format.match(/h/g) || []).length > 1;
			...
			return $.trim(format.replace(/(hh|h|mm|ss|tt|t)/g, function (e) {
				switch (e.toLowerCase()) {
					...
					case 'ss':
						return '00';
					case 't':
						return is24Hour ? '' : that.getT().toLowerCase();
					case 'tt':
						return is24Hour ? '' : that.getT();
				}
			}));
		};
		...
		this.getHour = function (is12Hour) {
			return is12Hour ? this.hour === 0 || this.hour === 12 ? 12 : this.hour % 12 : this.hour;
		};
		this.invert = function () {
			if (this.getT() === 'AM') this.setHour(this.getHour() + 12);else this.setHour(this.getHour() - 12);
		};
	...
	var MDTimePicker = function MDTimePicker(input, config) {
		var that = this;
		...
		picker.clockHolder.am.click(function () {
			if (that.selected.getT() !== 'AM') that.setT('am');
		});
		picker.clockHolder.pm.click(function () {
			if (that.selected.getT() !== 'PM') that.setT('pm');
		});
		picker.timeHolder.hour.click(function () {
			if (that.activeView !== 'hours') that.switchView('hours');
		});
		picker.timeHolder.minute.click(function () {
		...
		picker.clockHolder.buttonsHolder.btnOk.click(function () {
			...
			that.input.trigger($.Event('timechanged', { time: formatted.time, value: formatted.value })).trigger('onchange') // for ASP.Net postback
			.trigger('change');

			that.hide();
		});
		picker.clockHolder.buttonsHolder.btnCancel.click(function () {
			that.hide();
		});
	...
	MDTimePicker.prototype = {
		...
		setup: function setup(picker) {
			...
			// Setup time holder
			time.wrapper.append(time.hour).append(time.dots).append(time.minute).append(time.am_pm).appendTo(wrapper);
			...
			for (var i = 0; i < 60; i++) {
				var min = i < 10 ? '0' + i : i,
				    deg = (MIN_START_DEG + i * MIN_DEG_INCR) % END_DEG,
				    minute = $('<div class="mdtp__digit rotate-' + deg + '" data-minute="' + i + '"></div>');

				if (i % 5 === 0) minute.addClass('marker').html('<span>' + min + '</span>');else minute.html('<span></span>');
			...
			// Setup buttons
			clock.buttonsHolder.wrapper.append(clock.buttonsHolder.btnCancel).append(clock.buttonsHolder.btnOk).appendTo(clock.wrapper);
		...
		setValue: function setValue(value) {
			...
			this.input.val(formatted.value).attr('data-time', formatted.time).attr('value', formatted.value);
		},
		...
		parseTime: function parseTime(time, tFormat) {
			var that = this,
			    format = typeof tFormat === 'undefined' ? that.config.format : tFormat,
		...
		destroy: function destroy() {
			...
			that.input.removeData(MDTP_DATA).unbind('keydown').unbind('click').removeProp('readonly');
	...
	$.fn.mdtimepicker = function (config) {
		return $(this).each(function (idx, el) {
			...
			options = $.extend({}, $.fn.mdtimepicker.defaults, $that.data(), (typeof config === 'undefined' ? 'undefined' : _typeof(config)) === 'object' && config);
